...
{
    public interface IReadOnlyBehaviourSubject<T> : IObservable<T>
    {
        T Value { get; }
        bool TryGetValue([MaybeNullWhen(false)] out T value);
    }

    public interface IBehaviourSubject<T> : IReadOnlyBehaviourSubject<T>, ISubject<T>, IDisposable
    {
        new T Value { get; set; }
    }
    ...
    public class BehaviourSubject<T> : SubjectBase<T>, IBehaviourSubject<T>
    {
        #region Fields
        ...
        {
            ImmutableList<IObserver<T>> os = null;
        ...
        {
            private BehaviourSubject<T> _subject;
